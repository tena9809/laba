...
public:
    ...
    typedef unsigned int Key[ KEY_LEN ];
    typedef unsigned int Message[ MSG_LEN ];
...
private:
    typedef unsigned int SType[ 256 ];
...
static const unsigned int ROUND_COUNT = 16;
static const long long MOD_2_32 = (long long)( 2 ) << 31;

static const char K_MAP[ sizeof( CAST128::Key ) ] = {
        3,  2,  1,  0,
        7,  6,  5,  4,
        11, 10,  9,  8,
        15, 14, 13, 12
...
static char g( const CAST128::Key key, char i ) {
    return ( ( char* ) key )[ K_MAP[ i ] ];
}

static void splitI( unsigned int I, char* Ia, char* Ib, char* Ic, char* Id ) {
    *Ia = ( I >> 24 ) & 0xFF;
    *Ib = ( I >> 16 ) & 0xFF;
    *Ic = ( I >> 8  ) & 0xFF;
    *Id = ( I       ) & 0xFF;
...
static unsigned int sumMod2_32( unsigned int a, unsigned int b ) {
    return ( (long long)( a ) + (long long)( b ) ) % MOD_2_32;
}

static unsigned int subtractMod2_32( unsigned int a, unsigned int b ) {
    if( b <= a ) {
        return a - b;
    }
...
static unsigned int cyclicShift( unsigned int x, char shift ) {
    char s = shift % 32;
    return ( x << s ) | ( x >> ( 32 - s ) );
}
...
void CAST128::run( const CAST128::Key key, CAST128::Message msg, bool reverse ) {
    ...
    unsigned int L[ ROUND_COUNT + 1 ] = { 0 };
    L[ 0 ] = msg[ 0 ];

    unsigned int R[ ROUND_COUNT + 1 ] = { 0 };
    R[ 0 ] = msg[ 1 ];

    for( unsigned int i = 0; i < ROUND_COUNT; ++i ) {
        unsigned int rIndex = reverse ? ( ROUND_COUNT - 1 - i ) : i;
        unsigned int Kmi = K[ rIndex ];
        char  Kri = K[ 16 + rIndex ] & 0x1F;
